...
- 知识库模块已新增“网页导入”最小闭环：单网址抓取预览 -> 人工修正 -> 确认导入，反馈信息默认折叠并支持复制给 AI
- 网页导入当前复用 `ai_knowledge_document + ai_knowledge_chunk` 既有模型，不新建复杂网页抓取表；新增字段只补 `source_url`、`fetch_time`
- 网页抓取当前使用 `HttpClient + jsoup`，优先兼容静态或半静态文章详情页，不覆盖登录后页面、强反爬页面和复杂 JS 渲染页
- 知识库授权口径已改为“创建人默认有权限”：创建人默认拥有查看 / 上传 / 训练 / 分析该知识库的权限，不依赖显式授权记录
- 知识库授权管理不再仅限管理员：知识库创建人可在知识库页直接给本单位用户授权；超级管理员可给所有用户授权，不受单位限制
- 知识库页已新增“使用授权”区块，创建人默认权限会作为不可编辑的默认项展示，避免旧授权记录把创建人显示成无权限
- 知识库网页导入已补“动态页诊断 + 浏览器渲染兜底”：普通 HTTP 会先补浏览器常见请求头，再按 HTML 长度、标题缺失、正文极短和异步脚本/iframe/详情参数特征判断是否疑似动态页
- 若命中疑似动态页，当前会优先尝试本机可用的 Edge / Chrome / Chromium 无头浏览器 `--headless --dump-dom` 抓取渲染后的 DOM，再复用既有标题/正文评分逻辑择优
- 网页导入反馈信息当前已能区分普通 HTTP 与浏览器渲染两种模式，并记录是否触发兜底、普通/渲染正文长度、失败类型和下一步建议
- 网页导入普通 HTTP 当前即使在请求阶段抛异常，也不会直接丢失诊断信息：反馈里会记录请求客户端、请求开始时间、目标地址、最终地址、HTML 预览、异常类型/消息，以及超时、SSL、DNS、连接拒绝等标记
- 网页导入兜底触发条件当前已放宽为“请求失败、状态未获取、HTML 为空或过短、标题未获取、正文为空、普通抓取失败或疑似动态页”任一命中都会继续尝试浏览器渲染
